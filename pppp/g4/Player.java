		...
		for(int i=0; i<pipers.length; ++i)
		{
			...
			while(!remaining_pipers.isEmpty())
			{
				...
				for(int k=0; k<pipers[0].length; ++k)
				{
					...
					num_pipers++;
					nearby_pipers.add(p);
					for(Integer j : remaining_pipers)
						{
							if(!pipers_played[i][j]) {remaining_pipers.remove(j);}
							if(pipers_played[i][j] && getDistance(pipers[i][j], p)<=20)
							{
								num_pipers++;
								nearby_pipers.add(pipers[i][j]);
								remaining_pipers.remove(j);
							}
						}
					if(num_pipers>0)
					{
